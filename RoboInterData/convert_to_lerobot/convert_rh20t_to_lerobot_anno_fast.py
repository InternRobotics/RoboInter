"""
RH20T LMDB to LeRobot v2.1 Conversion Script (with Annotation Support) - Fast Version
...
def get_rh20t_features_config_with_anno() -> dict:
    """Get LeRobot v2.1 features configuration for RH20T dataset with annotations."""
    return {
        ...
        "other_information.gripper_command": {"dtype": "float64", "shape": (1,), "names": ["command"]},
        "other_information.observation_joint_position": {"dtype": "float64", "shape": (14,), "names": ["j1", "j2", "j3", "j4", "j5", "j6", "j7", "j8", "j9", "j10", "j11", "j12", "j13", "j14"]},
        ...
        "other_information.tcp_camera": {"dtype": "float64", "shape": (7,), "names": ["x", "y", "z", "qx", "qy", "qz", "qw"]},
        "other_information.tcp_base": {"dtype": "float64", "shape": (7,), "names": ["x", "y", "z", "qx", "qy", "qz", "qw"]},
        "other_information.gripper": {"dtype": "string", "shape": (1,), "names": ["json"]},
...
def resize_image_half(img: np.ndarray) -> np.ndarray:
    h, w = img.shape[:2]
    new_h, new_w = h // 2, w // 2
...
class RH20TFastDataset(LeRobotDataset):
...
def load_episode_and_generate_videos(
    ...
    try:
        ...
        for step_idx in range(num_steps):
            ...
            if resize_half:
                primary_img = resize_image_half(primary_img)
            ...
            if wrist_raw:
                ...
                if resize_half:
                    wrist_img = resize_image_half(wrist_img)
            else:
            ...
            # Handle observation_joint_position
            joint_pos = other_data.get("observation_joint_position")
            if joint_pos is None:
                joint_pos = np.zeros(14, dtype=np.float64)
            else:
                joint_pos = np.array(joint_pos, dtype=np.float64)
                if len(joint_pos) < 14:
                    joint_pos = np.pad(joint_pos, (0, 14 - len(joint_pos)), mode='constant')
                elif len(joint_pos) > 14:
                    joint_pos = joint_pos[:14]

            # Handle tcp_camera
            tcp_camera = other_data.get("tcp_camera")
            if tcp_camera is None:
                tcp_camera = np.zeros(7, dtype=np.float64)
            else:
                tcp_camera = np.array(tcp_camera, dtype=np.float64)
                if len(tcp_camera) < 7:
                    tcp_camera = np.pad(tcp_camera, (0, 7 - len(tcp_camera)), mode='constant')
                elif len(tcp_camera) > 7:
                    tcp_camera = tcp_camera[:7]

            # Handle tcp_base
            tcp_base = other_data.get("tcp_base")
            if tcp_base is None:
                tcp_base = np.zeros(7, dtype=np.float64)
            else:
                tcp_base = np.array(tcp_base, dtype=np.float64)
                if len(tcp_base) < 7:
                    tcp_base = np.pad(tcp_base, (0, 7 - len(tcp_base)), mode='constant')
                elif len(tcp_base) > 7:
                    tcp_base = tcp_base[:7]

            # Handle gripper
            gripper_raw = other_data.get("gripper")
            if gripper_raw is None:
                gripper_json = ""
            else:
                try:
                    gripper_json = json.dumps(convert_for_json(gripper_raw))
                except Exception:
                    gripper_json = ""
            ...
            frame_data = {
                ...
                "other_information.gripper_command": np.atleast_1d(int(other_data.get("gripper_command", 0))).astype(np.float64),
                "other_information.observation_joint_position": joint_pos,
                "other_information.observation_gripper_open_state": np.atleast_1d(int(other_data.get("observation_gripper_open_state", 0))).astype(np.float64),
                ...
                "other_information.tcp_camera": tcp_camera,
                "other_information.tcp_base": tcp_base,
                "other_information.gripper": gripper_json,
...
def convert_rh20t_to_lerobot_fast(
    ...
    """Convert RH20T dataset using fast parallel processing.
    ...
    error_log_file = error_log_dir / f"rh20t_skipped_episodes{shard_suffix}.txt"
    skipped_episodes = []

    # Use /tmp for temp videos to avoid conflicts with output directory
    temp_video_dir = Path("/tmp") / f"lerobot_temp_{output_path.name}"

    print("Scanning for RH20T episodes...")
    episode_dirs = get_rh20t_episode_list(input_path)
    total_episodes = len(episode_dirs)
    print(f"Found {total_episodes} RH20T episodes")
    ...
    first_result = load_episode_and_generate_videos(
        str(episode_dirs[0]), annotation_lmdb, qsheet_data, str(temp_video_dir), fps, resize_half
    )
    ...
    features = get_rh20t_features_config_with_anno()
    ...
    dataset = RH20TFastDataset.create(
    ...
    if resize_half:
        print("Images will be resized to half of original size")
            ...
            for ep_dir in batch_dirs:
                futures.append(
                    executor.submit(
                        ...
                        str(ep_dir), annotation_lmdb, qsheet_data, str(temp_video_dir), fps, resize_half
                    )
                )
    ...
    if skipped_episodes:
        with open(error_log_file, "w") as f:
            f.write(f"# RH20T Skipped Episodes{shard_suffix}\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert RH20T dataset to LeRobot v2.1 format (fast version)"
    )
    ...
    parser.add_argument("--qsheet_path", type=str,
                        default="")
    parser.add_argument("--num_threads", type=int, default=256, help="Number of threads for parallel processing")
    ...
    parser.add_argument("--no_resize", action="store_true", help="Do not resize images to half size")
    ...
    convert_rh20t_to_lerobot_fast(
        ...
        resize_half=not args.no_resize,
