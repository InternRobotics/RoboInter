...
from transformers.trainer_utils import seed_worker, HPSearchBackend, speed_metrics, TrainOutput
...
from transformers.utils import is_peft_available, is_torch_xla_available, is_apex_available, is_torch_xla_available, is_sagemaker_mp_enabled
...
def get_dataloader_sampler(dataloader):
    """
    Get the sampler used by a dataloader.

    Args:
        dataloader (`torch.utils.data.DataLoader`):
            The dataloader to get the sampler from.

    Returns:
        `torch.utils.data.Sampler`: The sampler used by the dataloader.
    """
    if not isinstance(dataloader, DataLoader):
        raise ValueError("The dataloader should be an instance of `torch.utils.data.DataLoader`.")
