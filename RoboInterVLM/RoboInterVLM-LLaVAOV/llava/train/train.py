...
from llava.data import data_list
...
class LazySupervisedDataset(Dataset):
    ...
    def process_image(self, image_file, is_local, resize, overwrite_image_aspect_ratio=None):
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "images" in sources[0]:
            ...
            if type(image_file) is list:
                ...
                image = [self.process_image(f, is_local, resize) for f in image_file]
                if len(image_file) > 1:
                    image = [self.process_image(f, is_local, resize, "pad") for f in image_file]
                    image = [[im[0], im[1], "image"] for im in image]
            else:
                image_file = os.path.join(image_folder, image_file)
                image = [self.process_image(image_file, is_local, resize)]
...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
    ...
    print(training_args.attn_implementation)
