...
from llavavla.model.framework.manipvla import QwenQFormerDiT
from eval._sim_cogact.adaptive_ensemble import AdaptiveEnsembler

app = Flask(__name__)


class RoboInterVLAService:
    def __init__(
        self,
        ...
        self.vla = QwenQFormerDiT.from_pretrained(
          saved_model_path
        )
        ...
        self.resize_image = transforms.Compose([transforms.Resize((224, 224))])
        self.args = args
        self.reset()
    ...
    def step(
        ...
        *args, **kwargs,
        ...
        if wrist_image:
            resized_wrist_image = self.resize_image(wrist_image)
        state = np.array(state).astype(np.float16)
        unnormed_actions, normalized_actions = self.vla.predict_action(
            image=resized_image,
            instruction=task_description,
            unnorm_key=self.unnorm_key,
            do_sample=False,
            wrist_image=resized_wrist_image,
            state=state,
...
parser.add_argument("--saved_model_path", type=str, default="path/to/pt files")
parser.add_argument("--unnorm_key", type=str, default="task_1")
...
inferencer = RoboInterVLAService(
...
def inference():
    try:
        ...
        wrist_image_file = request.files.get('wrist_images')
        if not wrist_image_file:
            return jsonify({"error": "No image file provided"}), 400
        wrist_image = Image.open(wrist_image_file.stream)
        ...
        if not state_file:
            return jsonify({"error": "No JSON file provided"}), 400
        try:
            state = json.load(state_file.stream)
        except json.JSONDecodeError as e:
            return jsonify({"error": f"Error loading JSON: {str(e)}"}), 400

        print("Input query:", input_query, "state", state_file)

        answer = inferencer.step(image, wrist_image, state, **input_query)
